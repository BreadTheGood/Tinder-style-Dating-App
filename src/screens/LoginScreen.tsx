    ...
    <div className="relative min-h-screen flex flex-col overflow-x-hidden overflow-y-auto custom-scrollbar" style={{ background: '#0d0d0f' }}>
      ...
      <div className="relative flex-1 flex flex-col justify-start mx-5">
        <div className="glass rounded-3xl p-6 pb-8">
          ...
          {(mode === 'login' || mode === 'register') && (
            <>
              <div className="relative flex items-center py-6">
                ...
                <span className="flex-shrink-0 mx-4 text-xs text-white/30 font-semibold uppercase tracking-widest">o</span>
                <div className="flex-grow border-t border-white/10"></div>
              </div>
              ...
              <div className="flex flex-col gap-3 mb-6">
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-3 py-3.5 rounded-xl font-semibold text-white/90 text-sm transition-all active:scale-95 disabled:opacity-50"
                ...
                >
                  <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
                    <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.15 2.95.93 3.78 2.04-3.19 1.9-2.69 5.98.54 7.21-.73 1.54-1.74 3.19-2.97 3.76zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.32 2.4-1.92 4.38-3.74 4.25z"/>
                  </svg>
                  Continuar con Apple
                </button>
              </div>
          ...
          {mode === 'login' && (
            <button onClick={() => setMode('register')} className="w-full mt-2 text-xs font-semibold text-white/50 hover:text-white transition-colors">
              ¿No tienes cuenta? <span className="text-[var(--theme-color-1)]">Regístrate</span>
            </button>
          )}
          {mode === 'register' && (
            <button onClick={() => setMode('login')} className="w-full mt-2 py-3 rounded-xl text-sm font-semibold text-white/70 hover:text-white hover:bg-white/5 transition-all">
              Volver
            </button>
          )}
        </div>
